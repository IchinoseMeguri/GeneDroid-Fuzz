...
class InputPolicy(object):
    ...
    中文：此类负责生成事件以刺激更多的应用程序行为
    它应该连续调用AppEventManager.send_event方法
    """
    ...
    def start(self, input_manager):
        ...
        while input_manager.enabled and self.action_count < input_manager.event_count:
            try:
                ...
                # 中文：确保第一个事件是转到HOME屏幕，第二个事件是启动应用程序
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def generate_event(self):
        ...
        # First try matching a state defined in the script（首先尝试匹配脚本中定义的状态）
...
# 把所有的控件都点一遍，再逐个寻找有状态转移的控件，进入下一个状态，再重复
# 这就是旧的
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    中文：DFS / BFS（根据search_method）策略来探索UFG（新）（UFG：用户行为图）
    """
    ...
    def generate_event_based_on_utg(self):
        ...
        if r < 50:
            from .heuristic_fuzz import IntentGenerator
            intent_generator = IntentGenerator()
            return IntentEvent(intent=intent_generator.generate_intent())
            ...
            # If couldn't find a exploration target, stop the app（如果找不到探索目标，请停止应用程序）
            stop_app_intent = self.app.get_stop_intent()
...
class UtgReplayPolicy(InputPolicy):
    ...
    def generate_event(self):
        ...
        # 首先获取当前设备状态，如果为None，等待5秒，然后重试，如果重试次数超过5次，返回BACK事件
        while self.event_idx < len(self.event_paths) and \
            ...
            while curr_event_idx < len(self.event_paths):
                # 从event_paths中获取当前事件路径
                event_path = self.event_paths[curr_event_idx]
                # 打开当前事件路径
                with open(event_path, "r") as f:
                    curr_event_idx += 1

                    # 尝试从当前事件路径中加载json数据，如果失败，记录日志，继续
                    try:
                    ...
                    # 如果当前事件路径中的start_state不等于当前状态，继续
                    if event_dict["start_state"] != current_state.state_str:
                        continue
                    # 如果当前事件路径中的start_state等于当前状态，且当前应用程序不在前台，将其带到前台
                    ...
                    # 如果当前事件路径中的start_state等于当前状态，且当前应用程序在前台，记录日志，重置重播尝试次数，返回事件
