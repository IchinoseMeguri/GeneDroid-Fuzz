...
class UTG(object):
    ...
    def __init__(self, device, app, random_input):
        ...
        # G：有向图，节点是状态，边是转换，边的属性是事件
        # G2：有向图，节点是结构，边是转换，边的属性是事件
        self.G = nx.DiGraph()
        self.G2 = nx.DiGraph()  # graph with same-structure states clustered

        self.transitions = []# 转换列表
        self.effective_event_strs = set()# 有效事件列表
        self.ineffective_event_strs = set()# 无效事件列表
        self.explored_state_strs = set()# 探索过的状态列表
        self.reached_state_strs = set()# 到达过的状态列表
        self.reached_activities = set()# 到达过的活动列表

        self.first_state = None# 第一个状态
        self.last_state = None# 最后一个状态
