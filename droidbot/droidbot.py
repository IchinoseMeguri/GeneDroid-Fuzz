...
# 中文：这个文件包含了droidbot的主类，它可以在AVD启动后，安装了app后，adb已经被正确设置后使用。
# 通过配置和创建一个droidbot实例，
# droidbot将会像人一样与AVD中的Android进行交互。
...
class DroidBot(object):
    ...
    def start(self): # 启动
        ...
        if not self.enabled:
            return
        self.logger.info("Starting DroidBot") # 开始droidbot
