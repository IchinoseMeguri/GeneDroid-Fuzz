...
# Unused currently, but should be useful.（目前未使用，但应该很有用。）
POSSIBLE_BROADCASTS = [
...
class EventLog(object):
    ...
    metadata = {
        ...
        'step.wait': 1,                     # time in seconds to wait after each input event
    }
    ...
    def start(self):
        ...
        if self.event.event_type == KEY_IntentEvent:  # 检测是否触发了intent事件
            self.trigger_intent_call(self.event)  # 触发intent调用
    ...
    def trigger_intent_call(self, event):
        ...
        intent = Intent()
        intent.cmd = event.intent
        intent.write_params_from_cmd()
        ...
        for intent in fuzzed_intents:
            ...
            time.sleep(1)
            dir=self.device.output_dir+"/intent_tset"
            device_state.save2dir(dir)

    def get_current_state(self):
        # 获取当前状态的方法，根据需要实现
        return self.device.get_current_state()
    ...
    def restore_state(self):
        ...
        # self.device.send_event(KeyEvent("BACK"))
        
        #     time.sleep(self.metadata['step.wait'])
        #     print("auto step_did: ",step_did)
        #     step_did+=10
