# helper file of droidbot（droidbot的辅助文件）
# it parses command arguments and send the options to droidbot（解析命令行参数并将选项发送到droidbot）
import argparse
...
def parse_args():
    ...
    # for distributed DroidBot（分布式DroidBot）
...
def main():
    ...
    中文：主函数
    中文：它根据cmd行中给出的参数启动droidbot
    """
    opts = parse_args()#解析命令行参数并将选项发送到droidbot
    import os
    ...
    if opts.distributed:
        ...
        if opts.distributed == "master":#主模式
            start_mode = "master"
        else:
            start_mode = "worker"#工作模式
    else:
        start_mode = "normal"#正常模式

    if start_mode == "master":#主模式
